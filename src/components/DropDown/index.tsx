...
export default function DropDown({ value, onChange }: DropDownProps) {
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
      <Menu.Button className={`uppercase tracking-wider w-44 inline-flex items-center justify-between gap-x-1 rounded-3xl px-4 py-2 text-sm font-bold text-baseText shadow-sm ring-2 ring-accentText hover:bg-accentText/30 
        ${value ? 'bg-accentText/60 text-white' : 'bg-white/20'}`}>
        {value || 'Select zodiac'} 
        <ChevronDownIcon className={`h-5 w-5 ${value ? ' text-white' : 'text-baseText'}`}/>
      </Menu.Button>
      </div>
                  ...
                  {({ active }) => (
                    <option
                      ...
                      className={classNames(
                        active ? 'uppercase bg-accentText text-white' : 'text-accentText uppercase',
