   ...
   useEffect(() => {
     const fetchData = async () => {
       ...
       try {
         ...
         console.error(error);
         setZodiacData({ name: '', about: '', date_range: '', compatibility: '', element: '', love: '', strengths: '', weaknesses: '', nature: '', career: '' });
       } finally {
         setIsLoading(false);
       }
     };
   ...
   return (
     <div className="flex flex-col justify-start w-full pt-10 p-4 font-titillium font-semibold text-black items-center">
       <div className="text-center">
         <h1 className="text-3xl md:text-5xl md:mt-16 font-extrabold tracking-widest uppercase">Select a zodiac sign</h1>
           ...
           <div>
             {zodiacData.compatibility === 'Error fetching data' ? (
               ...
               <div className='flex flex-col justify-center items-center max-w-3xl'>
                 ...
                 <h2 className="mt-5 text-5xl text-accentText font-extrabold tracking-widest uppercase text-center">{zodiacData.name}</h2>
                 <h2 className="text-center text-base text-accentText font-bold">{zodiacData.date_range}</h2>
                 <div className='grid grid-cols-2 border-accentText border-4 my-10 rounded-2xl'>
                   <div className="factBox rounded-tl-xl"><h3 className="font-bold">Strengths:</h3> {zodiacData.strengths}</div>                   
                   <div className="factBox rounded-tr-xl"><h3 className="font-bold">Weaknesses:</h3> {zodiacData.weaknesses}</div>
                   <div className="factBox rounded-bl-xl"><h3 className="font-bold">Compatibility:</h3> {zodiacData.compatibility}</div>
                   <div className="factBox rounded-br-xl"><h3 className="font-bold">Element:</h3> {zodiacData.element}</div>
                 </div>
                 <div className='px-2 md:p-0'>
                   <div className="mb-2 font-medium"><h3 className="font-bold">About:</h3> {zodiacData.about}</div>
                   <div className="mb-2 font-medium"><h3 className="font-bold">Love:</h3> {zodiacData.love}</div>
                   <div className="mb-2 font-medium"><h3 className="font-bold">Career:</h3> {zodiacData.career}</div>
                   <div className="mb-2 font-medium"><h3 className="font-bold">Nature:</h3> {zodiacData.nature}</div>
                 </div>
               </div>
             )}
           </div>
